...
struct Resolver<AssetType> {
    fs: crate::assets::file::FileSystem,
    ...
    Resolver<AssetType>
{
    ...
    fn new(path: crate::assets::file::ConsPath, path_prefix: &str) -> ggez::GameResult<Self> {
        let fs = path.fs();
        let path_prefix = path_prefix.to_string();

        // Load the resolver file's bytes
        let bytes = match crate::assets::file::try_bytes(path.into()) {
            Ok(bytes) => bytes,
            Err(e) => {
    ...
    fn try_new(path: crate::assets::file::ConsPath, path_prefix: &str) -> Option<Self> {
    ...
    fn try_get(
        ...
        crate::assets::file::try_bytes(crate::assets::file::Path::new(self.fs, path))
    }
    ...
    pub fn new() -> ggez::GameResult<Self> {
        Ok(Self {
            internal_sprite: Resolver::<Sprite>::try_new(
                crate::assets::file::ConsPath::new(
                    crate::assets::file::FileSystem::Internal,
                    "sprite/resolver.ron",
                ),
                "sprite/",
            ),
            external_sprite: Resolver::<Sprite>::try_new(
                crate::assets::file::ConsPath::new(
                    crate::assets::file::FileSystem::External,
                    "sprite/resolver.ron",
                ),
                "sprite/",
            ),
