...
impl Position {
    ...
    pub fn compute(
        ...
        match self {
            Position::Value(pt) => {
                crate::maths::Point::new(pt.x().compute(ctx), pt.y().compute(ctx))
            }
            Position::Anchor { anchor, offset } => {
                let offset =
                    crate::maths::Point::new(offset.x().compute(ctx), offset.y().compute(ctx));
                let drawable_size: crate::maths::Point = ctx.gfx.drawable_size().into();
